              ...
              <div class="row col-12 overflow-auto">
                <table class="table table-bordered table-hover">
                  ...
                  <tr>
                    ...
                    <th class="text-center text-nowrap">Tên tài khoản</th>
                    <th class="text-center text-nowrap">Base URL</th>
                    <th class="text-center text-nowrap">Account ID</th>
                    <th class="text-center text-nowrap">Inbox ID</th>
                  ...
                  <tr v-for="item in items" :key="item.id">
                    ...
                    <td class="align-middle">
                      <a :href="item.base_url" target="_blank" class="text-primary">
                        {{ item.base_url || '' }}
                        <i class="fa fa-external-link-alt ml-1"></i>
                      </a>
                    </td>
                    <td class="align-middle">{{ item.account_id || '' }}</td>
                    <td class="align-middle">{{ item.inbox_id || '' }}</td>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      window.ChatwootService.getList(apiParams)
    ...
    showDeleteConfirm(item) {
      this.showDeleteConfirmDialog(
        'Xác nhận xóa',
        `Bạn có chắc chắn muốn xóa tài khoản Chatwoot "${item.name}"?`,
        'Xóa',
        'Hủy'
      ).then(confirmed => {
        if (confirmed) {
    ...
    async performDelete(id) {
      try {
        ...
        await window.ChatwootService.delete(id);
        this.showSuccess('Xóa tài khoản Chatwoot thành công');
        this.debouncedFetchData();
      } catch (error) {
        this.showError((error.response && error.response.data && error.response.data.message) || error.message || 'Có lỗi xảy ra khi xóa tài khoản Chatwoot');
      } finally {
        this.is_loading = false;
      }
    },
    showDeleteConfirmDialog(title, message, confirmText = 'Xác nhận', cancelText = 'Hủy') {
