        ...
        <div class="col-md-12" :class="{'overlay-wrapper' : is_loading}">
          ...
          <div class="card">
            <div class="card-header d-flex align-items-center">
              <h3 class="card-title flex-grow-1">Quản lý Zalo Bots</h3>
              <button 
                v-if="hasPermission('zalo', 'create')"
                  ...
                  <tr v-for="item in items" :key="item.id">
                    <td class="text-center">
                      <div class="btn-group" role="group">
                        <button 
                          v-if="hasPermission('zalo', 'update')"
                        ...
                        <button 
                          v-if="hasPermission('zalo', 'delete')"
...
export default {
  ...
  methods: {
    ...
    async testConnection(bot) {
      try {
        const response = await window.ZaloService.testConnection(bot.id);
    ...
    async fetchData() {
      ...
      try {
        ...
        const response = await window.ZaloService.getList(apiParams);
        
        if (response.data.success) {
          this.items = response.data.data.bots || [];
        ...
        if (error.response?.status === 401) {
          ...
          window.ToastService.handleError(error, 'Có lỗi xảy ra khi tải danh sách Zalo bots');
        }
      } finally {
    ...
    showDeleteConfirm(item) {
      this.showConfirmDialog(
        'Xác nhận xóa',
        `Bạn có chắc chắn muốn xóa bot "${item.name}"?`,
        'Xóa',
        'Hủy'
      ).then(confirmed => {
        if (confirmed) {
    ...
    async performDelete(item) {
      try {
        this.is_loading = true;
        const response = await window.ZaloService.deleteBot(item.id);
        
        if (response.data.success) {
          window.ToastService.success(`Đã xóa bot "${item.name}" thành công`);
          this.fetchData();
        } else {
          window.ToastService.error(`Không thể xóa bot "${item.name}"`);
        }
      } catch (error) {
        window.ToastService.handleError(error, `Có lỗi xảy ra khi xóa bot "${item.name}"`);
      } finally {
        this.is_loading = false;
      }
    },
