        ...
        <div class="col-md-12" :class="{'overlay-wrapper' : is_loading}">
          ...
          <div class="card">
            <div class="card-header d-flex align-items-center">
              <h3 class="card-title flex-grow-1">Quản lý Zalo Official Account</h3>
              <button 
                v-if="hasPermission('zalo_oa', 'create')"
                ...
                <div class="col-md-4 col-sm-6">
                  <div class="form-group">
                    ...
                    <input 
                      ...
                      placeholder="Tìm kiếm theo tên OA..."
                      autocomplete="off"
                      data-lpignore="true"
              ...
              <div class="row col-12 overflow-auto">
                <table class="table table-bordered table-hover">
                  ...
                  <tr>
                    ...
                    <th class="text-center text-nowrap">Tên OA</th>
                    <th class="text-center text-nowrap">OA ID</th>
                    <th class="text-center text-nowrap">Access Token</th>
                    <th class="text-center text-nowrap">Secret Key</th>
                  ...
                  <tr v-for="item in items" :key="item.id">
                    <td class="text-center">
                      <div class="btn-group" role="group">
                        <button 
                          v-if="hasPermission('zalo_oa', 'update')"
                        ...
                        <button 
                          v-if="hasPermission('zalo_oa', 'read')"
                        ...
                        <button 
                          v-if="hasPermission('zalo_oa', 'delete')"
                    ...
                    <td class="align-middle">{{ item.oa_id || '' }}</td>
...
export default {
  ...
  methods: {
    ...
    async testConnection(oa) {
      try {
        const response = await window.ZaloOAService.testConnection(oa.id);
        
        if (response.data.success && response.data.data.connected) {
          this.showTestResult(oa, true, 'Kết nối thành công');
        } else {
          this.showTestResult(oa, false, response.data.data?.message || 'Kết nối thất bại');
        }
      } catch (error) {
        this.showTestResult(oa, false, 'Lỗi: ' + error.message);
      }
    },
    showTestResult(oa, success, message) {
    ...
    async fetchData() {
      ...
      try {
        ...
        if (apiParams.is_active === '') {
          ...
          apiParams.isActive = apiParams.is_active === 'true';
          delete apiParams.is_active;
        }
        
        const response = await window.ZaloOAService.getList(apiParams);
        
        if (response.data.success) {
          this.items = response.data.data.oas || response.data.data || [];
        ...
        if (error.response?.status === 401) {
          ...
          window.ToastService.handleError(error, 'Có lỗi xảy ra khi tải danh sách Zalo OA');
        }
      } finally {
    ...
    async handleEdit(item) {
      ...
      // Load full data from API for edit form (includes sensitive fields)
      try {
        const response = await window.ZaloOAService.getById(item.id);
        if (response.data.success) {
          this.selected_item = response.data.data;
        } else {
        ...
        this.selected_item = {...item};
        window.ToastService.handleError(error, 'Không thể tải thông tin chi tiết OA');
      }
    },
    showDeleteConfirm(item) {
      this.showConfirmDialog(
        'Xác nhận xóa',
        `Bạn có chắc chắn muốn xóa OA "${item.name}"?`,
        'Xóa',
        'Hủy'
      ).then(confirmed => {
        if (confirmed) {
    ...
    async performDelete(item) {
      try {
        this.is_loading = true;
        const response = await window.ZaloOAService.deleteOA(item.id);
        
        if (response.data.success) {
          window.ToastService.success(`Đã xóa OA "${item.name}" thành công`);
          this.fetchData();
        } else {
          window.ToastService.error(`Không thể xóa OA "${item.name}"`);
        }
      } catch (error) {
        window.ToastService.handleError(error, `Có lỗi xảy ra khi xóa OA "${item.name}"`);
      } finally {
        this.is_loading = false;
      }
    },
